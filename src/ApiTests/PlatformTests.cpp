...
void
testPlatform(std::function<void(cl_platform_id)>&& functor)
{
  cl_uint numPlatforms = 0;
...
TEST_CASE("Test clGetPlatformInfo invalid params", "[cl_platform]")
{
  testPlatform([](cl_platform_id platform) {
    REQUIRE(
      clGetPlatformInfo(nullptr, CL_PLATFORM_VENDOR, 0, nullptr, nullptr) ==
      CL_INVALID_PLATFORM);
    REQUIRE(
      clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, 0, nullptr, nullptr) ==
      CL_INVALID_VALUE);
    ...
    REQUIRE(
      clGetPlatformInfo(
        platform, CL_DEVICE_ADDRESS_BITS, data.size(), data.data(), nullptr) ==
      CL_INVALID_VALUE);
  });
}
...
TEST_CASE("Test clGetPlatformInfo", "[cl_platform]")
{
  testPlatform([](cl_platform_id platform) {
    ...
    REQUIRE(clGetPlatformInfo(
              platform, CL_PLATFORM_PROFILE, data.size(), data.data(), &size) ==
            CL_SUCCESS);
    ...
    REQUIRE(clGetPlatformInfo(
              platform, CL_PLATFORM_VERSION, data.size(), data.data(), &size) ==
            CL_SUCCESS);
    ...
    REQUIRE(clGetPlatformInfo(
              platform, CL_PLATFORM_NAME, data.size(), data.data(), &size) ==
            CL_SUCCESS);
    ...
    REQUIRE(clGetPlatformInfo(
              platform, CL_PLATFORM_VENDOR, data.size(), data.data(), &size) ==
            CL_SUCCESS);
    ...
    REQUIRE(
      clGetPlatformInfo(
        platform, CL_PLATFORM_EXTENSIONS, data.size(), data.data(), &size) ==
      CL_SUCCESS);
